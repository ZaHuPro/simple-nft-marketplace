...
let token;
contract("ERC721 token", function (accounts) {
  describe("deployment", async () => {
    it("deploy successfully", async () => {
      token = await KryptoBirdz.deployed();
  ...
  describe("contract metadata", async () => {
    it("Validate the symbol", async () => {
      expect(await token.symbol()).to.equal("KBZ");
    });
    it("Validate the name", async () => {
        expect(await token.name()).to.equal("KryptoBirdz");
      });
  });
});
// let token;
...
//     token = await KryptoBirdz.new(name, symbol)
